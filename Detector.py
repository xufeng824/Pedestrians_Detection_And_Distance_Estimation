"""BU SE740 Course Project
Pedestrians detection and distance estimation system
@Author: Xufeng Shen from Boston University College of Engineering
References:https://blog.csdn.net/m0_37811342/article/details/80394935
"""
...
class Yolo(object,):
    ...
    def _conv_layer(self, x, id, num_filters, filter_size, stride):
        ...
        weight = tf.Variable(tf.compat.v1.truncated_normal([filter_size, filter_size,
    ...
    def _fc_layer(self, x, id, num_out, activation=None):
        ...
        weight = tf.Variable(tf.compat.v1.truncated_normal([num_in, num_out], stddev=0.1))
    ...
    def show_results(self, image, results, imshow=True, deteted_boxes_file=None,
        ...
        for i in range(len(results)):
            ...
            if pix_person_height == 0:
               continue
            inches = self.distance_cal(focal_length, (pix_person_height * 2) / 0.15)
            binches = self.bdis(focal_length, pix_person_height * 2 / 0.15)
            if self.verbose:
                if results[i][0] == "person":
                    print("   class: %s, [x, y, w, h]=[%d, %d, %d, %d], confidence=%f" % (results[i][0],
                                                                                          x, y, w, h, results[i][-1]))
                    cv2.rectangle(img_cp, (x - w, y - h), (x + w, y + h), (0, 255, 0), 2)
                    cv2.rectangle(img_cp, (x - w, y - h - 40), (x + w, y - h), (125, 125, 125), -1)
                    cv2.putText(img_cp, results[i][0] + ' %.2f' % (inches * 0.0254) + 'm', (x - w + 5, y - h - 7),
                                cv2.FONT_HERSHEY_TRIPLEX, 0.8, (0, 0, 0), 1)
                if results[i][0] == "bottle":
                    print("   class: %s, [x, y, w, h]=[%d, %d, %d, %d], confidence=%f" % (results[i][0],
                                                                                          x, y, w, h, results[i][-1]))
                    cv2.rectangle(img_cp, (x - w, y - h), (x + w, y + h), (0, 255, 0), 2)
                    cv2.rectangle(img_cp, (x - w, y - h - 40), (x + w, y - h), (125, 125, 125), -1)
                    cv2.putText(img_cp, results[i][0] + ' %.2f' % (binches * 0.0254) + 'm', (x - w + 5, y - h - 7),
                                cv2.FONT_HERSHEY_TRIPLEX, 1.5, (0, 0, 0), 1)
                # print("   class: %s, [x, y, w, h]=[%d, %d, %d, %d], confidence=%f" % (results[i][0],
                #             x, y, w, h, results[i][-1]))
                # cv2.rectangle(img_cp, (x - w, y - h), (x + w, y + h), (0, 255, 0), 2)
                # cv2.rectangle(img_cp, (x - w, y - h - 40), (x + w, y - h), (125, 125, 125), -1)
                # cv2.putText(img_cp, results[i][0] + ' %.2f' % (inches * 0.0254), (x - w + 5, y - h - 7),
                #             cv2.FONT_HERSHEY_TRIPLEX, 1.5, (0, 0, 0), 1)

        cv2.imshow('Detection System', img_cp)
        cv2.waitKey(1)

    # pedestrians distance calculation function
    def distance_cal(self, focallength, perwid):

        #return (170 * focallength)/perwid
        return (66.9 * focallength)/perwid

    # bottle distance calculation function
    def bdis(self, focallength, perwid):

        return (6 * focallength)/perwid
...
if __name__ == "__main__":
    #focal = Focal()
    #focal_length = focal.focal_length()
    focal_length = 3327
    yolo_net = Yolo("./YOLO_small.ckpt")
    #cap = cv2.VideoCapture("IMG_2052.avi")
    cap = cv2.VideoCapture(0)
    while True:
        ret, img = cap.read()
        yolo_net.detect_from_file(img)
